...
DATASET="${DATASET:-data/versions/data_v3.1}"
DATA="${DATA:-$DATASET/data.yaml}"
WEIGHTS_DIR="${WEIGHTS_DIR:-pretrained_weights}"
RUNS_DIR="${RUNS_DIR:-runs}"
DATASET_VERSION="${DATASET_VERSION:-$(basename "$DATASET")}"
DATASET_TAG="${DATASET_TAG:-${DATASET_VERSION//./_}}"
REPORTS_DIR="${REPORTS_DIR:-reports/waruav_${DATASET_TAG#data_}}"
...
BATCH_11S="${BATCH_11S:-8}"
BATCH_11M="${BATCH_11M:-8}"
BATCH_26S="${BATCH_26S:-8}"
BATCH_26M="${BATCH_26M:-8}"
...
for spec in "${MODEL_SPECS[@]}"; do
  ...
  batch="${spec##*:}"
  model_id="${model%.pt}"
  run_name="${model_id}_${DATASET_TAG}_img${IMGSZ}_b${batch}_waruav"
